...
static bool scan_automatic_semicolon(TSLexer *lexer, const bool *valid_symbols) {
  ...
  if (sameline) {
    ...
    if (valid_symbols[SAME_LINE_BOUNDARY] && !valid_symbols[MODIFIER_RUN_HINT] &&
      ...
      int32_t c = lexer->lookahead;
      // 'c' words stay with the internal lexer while a primary constructor
      // is expected: 'class B constructor(x: Int)' must keep its header
      // reading, and the later PRIMARY_CONSTRUCTOR_KEYWORD branch needs an
      // unconsumed stream.
      if (c == 'v' || c == 'f' || c == 'o' || c == 't' || c == 'i' ||
          c == 'g' || c == 's' ||
          (c == 'c' && !valid_symbols[PRIMARY_CONSTRUCTOR_KEYWORD])) {
        char word[20];
        buffer_word(lexer, word);
